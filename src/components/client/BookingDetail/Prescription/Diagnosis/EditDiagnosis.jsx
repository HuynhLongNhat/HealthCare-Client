...
const EditDiagnosis = ({ show, handleClose, fetch }) => {
  ...
  const fetchDiagnosisData = async () => {
    ...
    if (res.EC === 0) {
      setDiagnosis(res.DT);
      const data = res.DT.diagnosis
;
      form.setValue("symptoms", data.symptoms || "");
      form.setValue("diagnosis", data.diagnosis || "");
      form.setValue("treatmentPlan", data.treatment_plan || "");
  ...
  const onSubmit = async (data) => {
    try {
      ...
      const res = await updateDiagnosis(diagnosis.diagnosis.id, payload);
      if (res.EC === 0) {
